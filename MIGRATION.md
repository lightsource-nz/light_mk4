...
|---|---|---|---|
| Waveshare RP2350-Touch-LCD-3.49 (AXS15231B QSPI) | `light-rp2::qspi` + `light-display::axs15231b` + `light-input::axs15231b` + `light_mk4_touch349` | ported | hw-verified 2026-08-31: panel lit first flash, taps/drags/swipe-back on the glass, IMU axis map measured. First QSPI panel, first RP2350B board, first upper-bank GPIO use. Two driver findings: the touch half is consume-on-read, so release is inferred from report silence, never read; and the panel ignores partial windowing (per-row RAMWR wedges it, RASET row offsets land at row 0), so every push is a full frame. Peripherals filled in 2026-08-31: battery ADC + the SYS_EN power latch (`light-rp2::adc`, first ADC use), PCF85063A RTC (new `light-rtc` crate, hw-verified keeping time across power loss), ES8311 audio (new `light-audio` crate + `light-rp2::i2s` -- codec as I2S master, ping-pong DMA stream after polled FIFO feeding chopped audibly), and the TF slot (new `light-sd` crate over the new `SpiBus` trait: SPI-mode identify + block reads, hw-verified against a 64 GB SDXC). PSRAM: MEASURED ABSENT -- the demo pack's library is family boilerplate, not fitment; the SDK auto-detect stays wired for a fitted variant. Filesystem landed 2026-09-03: `light-fs` (FAT16/32 over the new `BlockDevice` trait) hw-verified against a Raspberry Pi boot SD in this slot -- `fs info|ls|cat|write|rm|mv|mkdir|trunc` on the console: read, write (create/append/seek, write-through with FAT mirroring, LFN read) and the directory operations (remove/truncate/rename/mkdir/rmdir), all round-tripped on the card. Dictaphone landed 2026-09-03, end-to-end voice loop verified at true pitch 2026-09-04: ES8311 encoder (`mic_enable`/`mic_config`) + PIO I2S capture (`light-rp2::i2s` capture SM + ping-pong DMA) + `rec`/`play` WAV through light-fs. Capture rate proven by soak (47.9 KB/s over 32 s), playback rate proven by log timestamps (2.000 s sine in 1.975 s) after fixing a 2x bug: the dout PIO consumes ONE word per frame (top half = left slot) but both producers pushed two words per sample against a wrong header comment -- every playback before the fix ran an octave down. `rec null` drains the mic pipeline with the card out of the path (the bisect that cleared the firmware when the bench's dying SD card froze sustained writes) |
| Dictaphone (second app on the RP2350-Touch-LCD-3.49) | `light_mk4_dictaphone` | ported | The first board with two separately flashable applications: same board, shell and build tree as `light_mk4_touch349`, its own executable and UF2 (`flash.ps1 -Target light_mk4_dictaphone`). A voice recorder with a dedicated touch interface over the codec + TF-slot leg: live status line, record/stop, play-last, a list of the newest eight auto-numbered `REC_NNNN.WAV` takes. Drove three framework changes: light-ui's `set_text` (owned per-widget runtime text), light-fs's first-cluster validation (corrupt volumes answer `BadChain` instead of panicking), and -- found here, fixed in the shell for every device-role board -- core 1's stack relocated out of SCRATCH_X, where deep core-0 filesystem call chains had been trampling it and killing the console silently. Hw-verified 2026-09-04 end to end on the glass; the diag row carries a core-1 heartbeat and a stack watermark |
