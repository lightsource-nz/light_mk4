//! The dictaphone: a voice recorder with a dedicated touch interface -- the APPLICATION,
//! with no hardware in it. Recordings are auto-named `REC_NNNN.WAV` (8.3: the filesystem
//! writes short names only) and land as ordinary mono 16-bit WAV any desktop player opens.
//!
...
//! [`LIST_ROWS`] takes, tap to play. The full diagnostic console (`rec`/`play`/`fs`/
//! `tone`/`micdbg`/...) stays: it is the same machinery the buttons drive.
//!
//! A tangible dictaphone is a hardware-bound module that constructs the concrete parts and
//! owns everything this crate must not: pins, the display driver, the touch controller,
//! the I2S transport and its buffers, the card, the shell ABI and the panic handler. The
//! seams, in order of appearance:
...
//! - [`dictaphone_pages!`] expands the page tree in the board crate, where the event type
...
pub use light_ui::{scroll, Desc, Page};
...
/// The dictaphone's two pages, as statics in the BOARD crate: `PAGE_MAIN` and
/// `PAGE_FILES`, with the board's row gap and touch-target heights baked in -- the metrics
/// are hardware (finger size against pixel density), the tree is not.
///
/// ```ignore
/// light_app_dictaphone::dictaphone_pages! {
///         event: AppEvent, row_gap: 6, list_min_row: 56, status_min_h: 40, rec_min_h: 88
/// }
/// ```
#[macro_export]
macro_rules! dictaphone_pages {
        (event: $event:ty, row_gap: $row_gap:expr, list_min_row: $list_min_row:expr, status_min_h: $status_min_h:expr, rec_min_h: $rec_min_h:expr) => {
                static LBL_STATUS: $crate::Desc<$event> = $crate::Desc::label("ready").tag($crate::TAG_STATUS).min_size(0, $status_min_h);
                ...
                static MAIN_WINDOW: $crate::Desc<$event> = $crate::Desc::window("Dictaphone").stack($row_gap).children(&[&LBL_STATUS, &BTN_REC, &BTN_PLAY, &BTN_FILES]);
...
impl<D: DisplayDriver, C: Clock, X: Copy + core::fmt::Debug + 'static> DisplayMod<D, C, X> {
        pub fn new(
                ...
                Self { display, layer, font, ui, clock, cfg, bus, sub, mode: RenderMode::Normal, status: AudioStatus::Idle, drag_reported: false, draw_us_max: 0, push_us_max: 0, push_started_us: None }
        }
        ...
        /// The status line: what the audio module is doing, in the terms a user reads.
...
pub struct AudioMod<S: Store, B: I2cBus, A: AudioStream, P: OutputPin, C: Clock, X: Copy + 'static> {
        ...
        /// One stream-buffer's worth of file bytes, bulk-read per refill so the DAC ring
        /// is filled from RAM, not per-sample off the card (which starved it: 33 underruns
        /// in a measured playback). Size it to one full refill.
...
impl<S: Store, B: I2cBus, A: AudioStream, P: OutputPin, C: Clock, X: Copy + core::fmt::Debug + 'static> AudioMod<S, B, A, P, C, X> {
        ...
        fn rec_stop(&mut self) {
                ...
                match patch {
                        Ok(()) => info!("rec: stopped -- {} B of audio, ~{} ms", data, data / 2 * 1000 / self.sample_hz),
        ...
        fn play_start(&mut self, path: &str) {
                ...
                for _ in 0..16 {
                        ...
                        if &ch[..4] == b"fmt " && len >= 16 {
                                ...
                                let ms = (data_end - data_start) / (u32::from(channels) * 2) * 1000 / self.sample_hz;
...
impl<S: Store, B: I2cBus, A: AudioStream, P: OutputPin, C: Clock, X: Copy + core::fmt::Debug + 'static> Module for AudioMod<S, B, A, P, C, X> {
        ...
        fn poll(&mut self) -> Poll {
                ...
                if let Some(play) = self.play.as_mut() {
                        ...
                        //   pre-borrowed so the closure captures fields disjoint from the stream
